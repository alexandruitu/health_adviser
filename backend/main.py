...
import strava as _strava_mod
import gdrive as _gdrive_mod
import garmin as _garmin_mod
import adviser as _adviser_mod
import ai_search as _ai_search_mod
...
def readiness_endpoint(start: Optional[str] = None, end: Optional[str] = None):
    """Personalized Recovery Readiness Index combining HRV, sleep score, body
    battery and training stress balance."""
    from readiness import compute_readiness_series
    from datetime import date

    start_d = date.fromisoformat(start) if start else date(date.today().year, 1, 1)
    end_d   = date.fromisoformat(end)   if end   else date.today()
    return compute_readiness_series(start_d, end_d)


@app.get("/api/sleep/wellness")
def sleep_wellness(start: Optional[str] = None, end: Optional[str] = None):
    """Return Garmin sleep wellness metrics per night: score, overnight HR,
    respiration, SpO2, restless moments, body battery change, skin temp."""
    from db import _db
    import pandas as pd
    ...
    METRICS = [
        "GarminSleepScore",
        "GarminSleepHR",
        "GarminSleepRespiration",
        "GarminSleepRespirationLow",
        "GarminSleepSpO2",
        "GarminSleepSpO2Low",
        "GarminSleepRestless",
        "GarminBodyBatteryDuringSleep",   # replaces deprecated GarminBodyBatteryChange
        "GarminBodyBatteryChange",         # keep for backward compat
        "GarminAvgSleepStress",
        "GarminSkinTempChange",
    ]
    ...
    rows = conn.execute(f"""
        SELECT metric_name,
               date(datetime(start_ts, 'unixepoch')) AS day,
               AVG(value) AS val
        FROM metrics
    ...
    # Build date → {metric: val} dict
    by_date: dict = {}
    for metric_name, day, val in rows:
        by_date.setdefault(day, {"date": day})[metric_name] = round(val, 2) if val is not None else None
...
def strava_sync(background_tasks: BackgroundTasks, force: bool = False):
    if _strava_mod._sync_job.get("status") == "running":
...
def gdrive_sync(background_tasks: BackgroundTasks):
    if _gdrive_mod._gdrive_sync_job.get("status") == "running":
...
def gdrive_sync_status():
    return _gdrive_mod._gdrive_sync_job
...
def garmin_sync(background_tasks: BackgroundTasks, force: bool = False):
    if _garmin_mod._garmin_sync_job.get("status") == "running":
        return {"status": "already_running"}
...
def garmin_sync_status():
    return _garmin_mod._garmin_sync_job
...
@app.post("/api/adviser/assess")
def adviser_assess(req: AdviserRequest):
    """Stream an AI-powered health assessment for the given tab data."""
    try:
        return _adviser_mod.stream_assessment(
            tab=req.tab,
...
class AdviserFollowupRequest(BaseModel):
    tab: str
    start: str
    end: str
    data: dict
...
@app.post("/api/adviser/followup")
def adviser_followup(req: AdviserFollowupRequest):
    """Stream a follow-up response given the full conversation history."""
    try:
        return _adviser_mod.stream_followup(
...
@app.post("/api/activities/ai_search")
def activities_ai_search(req: AISearchRequest):
    """Use Claude to interpret a natural language query and search activities."""
    import math as _math
    try:
        # Step 1: Parse natural language into structured filters
        filters = _ai_search_mod.parse_query(req.query)

        # Step 2: Apply filters to workout data
        wo = _workouts().copy()
        wo["startDate"] = pd.to_datetime(wo["startDate"], errors="coerce")

        if filters.get("date_start"):
            wo = wo[wo["startDate"] >= pd.Timestamp(filters["date_start"])]
        if filters.get("date_end"):
            wo = wo[wo["startDate"] <= pd.Timestamp(filters["date_end"]) + pd.Timedelta(days=1)]
        if filters.get("sport") and filters["sport"] != "all":
            wo = wo[wo["workoutType"].str.contains(filters["sport"], case=False, na=False)]
        if filters.get("name_contains"):
            wo = wo[wo["activity_name"].fillna("").str.contains(filters["name_contains"], case=False)]

        # Compute pace for filtering
        wo["_dist"] = pd.to_numeric(wo.get("distance"), errors="coerce")
        wo["_dur"]  = pd.to_numeric(wo.get("duration_min"), errors="coerce")
        wo["_pace"] = wo["_dur"] / wo["_dist"].replace(0, float("nan"))
        wo["_hr"]   = pd.to_numeric(wo.get("avg_hr"), errors="coerce")
        wo["_elev"] = pd.to_numeric(wo.get("elevation_m"), errors="coerce")
        wo["_watts"] = pd.to_numeric(wo.get("avg_watts"), errors="coerce")
        wo["_suffer"] = pd.to_numeric(wo.get("suffer_score"), errors="coerce")

        if filters.get("min_distance_km"):
            wo = wo[wo["_dist"] >= filters["min_distance_km"]]
        if filters.get("max_distance_km"):
            wo = wo[wo["_dist"] <= filters["max_distance_km"]]
        if filters.get("min_duration_min"):
            wo = wo[wo["_dur"] >= filters["min_duration_min"]]
        if filters.get("max_duration_min"):
            wo = wo[wo["_dur"] <= filters["max_duration_min"]]
        if filters.get("min_pace_min_km"):
            wo = wo[wo["_pace"] >= filters["min_pace_min_km"]]
        if filters.get("max_pace_min_km"):
            wo = wo[wo["_pace"] <= filters["max_pace_min_km"]]
        if filters.get("min_avg_hr"):
            wo = wo[wo["_hr"] >= filters["min_avg_hr"]]
        if filters.get("max_avg_hr"):
            wo = wo[wo["_hr"] <= filters["max_avg_hr"]]
        if filters.get("min_elevation_m"):
            wo = wo[wo["_elev"] >= filters["min_elevation_m"]]
        if filters.get("min_avg_watts"):
            wo = wo[wo["_watts"] >= filters["min_avg_watts"]]

        # Sort
        sort_map = {
            "date": "startDate", "distance_km": "_dist", "duration_min": "_dur",
            "pace_min_km": "_pace", "avg_hr": "_hr", "elevation_m": "_elev",
            "suffer_score": "_suffer", "avg_watts": "_watts",
        }
        sort_col = sort_map.get(filters.get("sort_by", "date"), "startDate")
        sort_asc = filters.get("sort_dir", "desc") == "asc"
        wo = wo.sort_values(sort_col, ascending=sort_asc, na_position="last")
        ...
        records = []
        for _, r in wo.iterrows():
            pace_s = safe(r.get("_pace"))
            records.append({
                "date":        str(r["startDate"])[:10],
                "name":        safe(r.get("activity_name")) or "",
                "type":        safe(r.get("workoutType")) or "",
                "source":      safe(r.get("sourceName")) or "",
                "distance_km": safe(r.get("_dist")),
                "duration_min": safe(r.get("_dur")),
                "pace_min_km": pace_s,
                "avg_hr":      safe(r.get("_hr")),
                "max_hr":      safe(r.get("max_hr")),
                "elevation_m": safe(r.get("_elev")),
                "suffer_score": safe(r.get("_suffer")),
                "avg_cadence": safe(r.get("avg_cadence")),
                "avg_watts":   safe(r.get("_watts")),
                "trainer":     False,
            })
        ...
        summary = ""
        try:
            summary = _ai_search_mod.generate_summary(req.query, filters, records, total)
        except Exception:
            summary = f"Found {total} matching activities."
...
# ─── Global Advisor (sidebar Ask the Advisor) ────────────────────────────────
...
def advisor_ask(req: AdvisorRequest):
    ...
    from biomarkers import _load_biomarkers_config
    ...
    if not api_key:
        raise HTTPException(status_code=503, detail="Anthropic API key not configured in biomarkers_config.json")

    conn = _db()
    ts_30d = time.time() - 30 * 86400

    metric_summary = {}
    for metric in ["RestingHeartRate", "HeartRateVariabilitySDNN", "StepCount",
                   "ActiveEnergyBurned", "BodyMass", "BodyFatPercentage", "VO2Max"]:
        row = conn.execute(
            "SELECT AVG(value), unit FROM metrics WHERE metric_name=? AND start_ts>?",
            (metric, ts_30d)
        ).fetchone()
        if row and row[0] is not None:
            metric_summary[metric] = {"avg": round(row[0], 2), "unit": row[1] or ""}

    workout_rows = conn.execute("""
        SELECT workout_type, COUNT(*) as count,
               ROUND(AVG(duration_min),1) as avg_dur,
               ROUND(SUM(distance_km),1) as total_km
        FROM workouts WHERE start_ts>?
        GROUP BY workout_type ORDER BY count DESC
    """, (ts_30d,)).fetchall()
    workouts = [{"type": r[0], "sessions": r[1], "avg_duration_min": r[2], "total_km": r[3]}
                for r in workout_rows]

    sleep_rows = conn.execute("""
        SELECT stage, ROUND(AVG(duration_min)/60.0, 2) as avg_hours
        FROM sleep WHERE start_ts>?
        GROUP BY stage
    """, (ts_30d,)).fetchall()
    sleep_summary = {r[0]: r[1] for r in sleep_rows if r[1] is not None}

    bio_rows = conn.execute("""
        SELECT b.marker_canonical, b.value, b.unit, b.status, b.category, b.test_date
        FROM biomarkers b
        JOIN (
            SELECT marker_canonical, MAX(test_date) as latest
            FROM biomarkers GROUP BY marker_canonical
        ) latest ON b.marker_canonical=latest.marker_canonical AND b.test_date=latest.latest
        ORDER BY b.category, b.marker_canonical
    """).fetchall()
    biomarkers = [{"name": r[0], "value": r[1], "unit": r[2], "status": r[3],
                   "category": r[4], "date": r[5]} for r in bio_rows]
    ...
    if metric_summary:
        context_parts.append(f"## Health Metrics (30-day averages)\n{json.dumps(metric_summary, indent=2)}")
    if workouts:
        context_parts.append(f"## Recent Workouts (last 30 days)\n{json.dumps(workouts, indent=2)}")
    if sleep_summary:
        context_parts.append(f"## Sleep (30-day averages, hours per stage)\n{json.dumps(sleep_summary, indent=2)}")
    if biomarkers:
        context_parts.append(f"## Latest Lab Results\n{json.dumps(biomarkers, indent=2)}")
    ...
    message = client.messages.create(
        ...
        max_tokens=2048,
        system="""You are a knowledgeable health and longevity advisor with expertise in exercise physiology, nutrition, sleep science, and preventive medicine. You have access to the user's personal health data from Apple Health, Garmin, Strava, and lab results.

Provide personalized, evidence-based advice based on the actual data. Be specific and reference the user's real numbers when relevant. Structure your response clearly with headers where appropriate. Always note when something warrants medical attention from a professional.""",
        messages=[{
            "role": "user",
            "content": f"Here is my current health data:\n\n{health_context}\n\n---\n\nMy question: {question}"
        }]
    )
