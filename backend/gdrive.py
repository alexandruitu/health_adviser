...
def _gdrive_folder_id(headers: dict, folder_path: str) -> Optional[str]:
    """Resolve 'Health Auto Export/AutoExport/HealthMetrics' → Google Drive folder ID.
    Handles folder names with slashes by trying different split points."""
    parts = [p for p in folder_path.split("/") if p]

    # Try progressively: first look for the full path as nested folders,
    # then try combining parts as folder names (in case folder names contain "/")

    # Strategy 1: Try full nested path (standard case)
    parent_id = "root"
    for part in parts:
        ...
        if not files:
            break  # This part doesn't exist as a subfolder
        parent_id = files[0]["id"]
    else:
        # All parts found as nested folders
        return parent_id

    # Strategy 2: Try combining parts as single folder names (handles "/" in names)
    parent_id = "root"
    for num_parts in range(len(parts), 0, -1):
        # Combine first num_parts into folder name, rest as subfolders
        folder_name = "/".join(parts[:num_parts])
        remaining = parts[num_parts:]

        # Find the combined folder name
        r = httpx.get(GDRIVE_FILES_API, headers=headers, params={
        ...
        # Then traverse remaining subfolders
        success = True
