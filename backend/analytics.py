...
def _daily() -> pd.DataFrame:
    ...
    for metric, agg in METRIC_AGG.items():
        if metric == "StepCount":
            # Exclude pure Garmin Connect source — it reports raw daily step totals
            # that double-count steps already captured by the iPhone/Watch pedometer.
            # 'iPhone|Connect' (Garmin watch synced via iPhone) is kept; it's the
            # primary source for Garmin watch step data and doesn't overlap with
            # the native iPhone pedometer in the same way.
            q = f"""
    ...
    # HeartRate mean / min / max
    # Prefer Garmin daily stats (GarminHeartRate_*) which capture full-day HR
    # including workouts. Fall back to raw HeartRate aggregation where Garmin
    # data is unavailable.
    for suffix, agg, garmin_metric in [
        ("mean", "AVG", "GarminHeartRate_mean"),
        ("min",  "MIN", "GarminHeartRate_min"),
        ("max",  "MAX", "GarminHeartRate_max"),
    ]:
        # Garmin daily stats
        q_garmin_hr = """
            SELECT date(datetime(start_ts, 'unixepoch')) AS day, value AS val
            FROM metrics WHERE metric_name = ? GROUP BY day
        """
        garmin_tmp = pd.read_sql_query(q_garmin_hr, conn, params=(garmin_metric,))
        garmin_tmp.columns = ["date", f"HeartRate_{suffix}"]

        # Raw HeartRate aggregation (Apple Health / Connect / other)
        q_raw = f"""
            SELECT date(datetime(start_ts, 'unixepoch')) AS day,
                   {agg}(value) AS val
            FROM metrics
            ...
            GROUP BY day
        """
        raw_tmp = pd.read_sql_query(q_raw, conn)
        raw_tmp.columns = ["date", f"HeartRate_{suffix}"]

        # Merge: Garmin first, fill gaps with raw
        combined = garmin_tmp.set_index("date").combine_first(raw_tmp.set_index("date")).reset_index()
...
def _sleep() -> pd.DataFrame:
    ...
    conn = _db()
    # Connect stores individual sleep-stage segments (many rows per stage per night),
    # Garmin stores one summary row per stage.  SUM within each source first to get
    # per-source totals, then AVG across sources to deduplicate dual-reported nights.
    # Prefer Garmin sleep data when available for a given night — it's more
    # accurate (dedicated sensor, single summary row per stage).  Fall back to
    # AVG across other sources for nights where Garmin has no data.
    df = pd.read_sql_query(
        """SELECT date, stage,
                  CASE WHEN SUM(CASE WHEN source = 'Garmin' THEN 1 ELSE 0 END) > 0
                       THEN SUM(CASE WHEN source = 'Garmin' THEN total_min ELSE 0 END)
                       ELSE AVG(total_min)
                  END AS duration_min
           FROM (
               SELECT date, stage, source, SUM(duration_min) AS total_min
               FROM sleep
               WHERE stage IN ('Core','Deep','REM','Awake','InBed','Unspecified')
               GROUP BY date, stage, source
           )
           GROUP BY date, stage""",
        conn
    )
