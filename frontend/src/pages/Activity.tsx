  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "0.75rem" }}>
        <h1 style={{ color: "#e2e8f0", fontSize: "1.25rem", fontWeight: 600, margin: 0 }}>Training</h1>
      ...
      <AdviserPanel
        ...
        gatherData={useCallback(() => ({
          period: `${start} to ${end}`,
          running: { sessions: runSessions, hours: +(totalRunMin / 60).toFixed(1), km: +totalRunKm.toFixed(0), elevation_m: totalRunElev, longest_min: longestRun },
          cycling: { sessions: cycSessions, hours: +(totalCycMin / 60).toFixed(1), km: +totalCycKm.toFixed(0), elevation_m: totalCycElev, longest_min: longestRide },
          fitness: { ctl: latestCTL, tsb: latestTSB, date: latestDate },
          strava: { avg_run_hr: +avgRunHR.toFixed(0), avg_run_pace: +avgRunPace.toFixed(2), total_suffer: totalSuffer, run_elev: insRunElev, cyc_elev: insCycElev },
        }), [start, end, runSessions, totalRunMin, totalRunKm, totalRunElev, longestRun, cycSessions, totalCycMin, totalCycKm, totalCycElev, longestRide, latestCTL, latestTSB, latestDate, avgRunHR, avgRunPace, totalSuffer, insRunElev, insCycElev])}
      />
