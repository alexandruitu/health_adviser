...
function MiniSparkline({ history, color, refMin, refMax, unit }: {
  history: BiomarkerReading[]; color: string; refMin: number | null; refMax: number | null; unit: string;
}) {
  if (history.length < 2) return null;
  const lastDate = history[history.length - 1].test_date;
  const data = history.map(r => ({
    d: new Date(r.test_date).toLocaleDateString("en", { year: "2-digit", month: "short" }),
    year: "'" + new Date(r.test_date).getFullYear().toString().slice(2),
    v: r.value,
    isLatest: r.test_date === lastDate,
  }));
  return (
    <div style={{ width: "100%", height: 56, marginTop: "0.3rem" }}>
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data} margin={{ top: 6, right: 14, bottom: 16, left: 14 }}>
          {refMin != null && <ReferenceLine y={refMin} stroke="#10b98125" strokeDasharray="2 2" />}
          {refMax != null && <ReferenceLine y={refMax} stroke="#f9731625" strokeDasharray="2 2" />}
          ...
          <Line type="monotone" dataKey="v" stroke={color} strokeWidth={1.5}
            dot={(props: { cx?: number; cy?: number; index?: number; payload?: { isLatest: boolean } }) => {
              const { cx, cy, payload } = props;
              if (cx == null || cy == null || !payload) return <g />;
              return (
                <circle cx={cx} cy={cy} r={payload.isLatest ? 4 : 2.5} fill={color}
                  stroke={payload.isLatest ? "#e2e8f0" : "none"} strokeWidth={payload.isLatest ? 1.5 : 0} />
              );
            }}
            activeDot={{ r: 5, fill: color, stroke: "#fff", strokeWidth: 2 }}
          />
          <Tooltip
            contentStyle={{ backgroundColor: "#1a1d27", border: "1px solid #2a2d3a", borderRadius: "0.375rem", padding: "6px 10px" }}
            labelStyle={{ color: "#94a3b8", fontSize: "0.72rem", fontWeight: 600, marginBottom: 2 }}
            formatter={(v: number) => {
              const formatted = v % 1 === 0 ? v.toString() : v.toFixed(2).replace(/\.?0+$/, "");
...
function MarkerCard({ m, history, onClick }: { m: BiomarkerReading; history: BiomarkerReading[]; onClick: () => void }) {
  ...
  const prevReading = sorted.length >= 2 ? sorted[sorted.length - 2] : null;
  const delta = prevReading && prevReading.value !== 0
    ? ((m.value - prevReading.value) / Math.abs(prevReading.value)) * 100
    : null;
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "0.5rem" }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: "0.75rem", fontWeight: 600, color: "#cbd5e1", lineHeight: 1.2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {m.marker_canonical}
          </div>
          <div style={{ fontSize: "0.62rem", color: "#475569", marginTop: "2px" }}>{fmtDate(m.test_date)}</div>
        </div>
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: "0.25rem", justifyContent: "flex-end" }}>
            <span style={{ fontSize: "0.85rem", fontWeight: 700, color }}>{fmtVal(m.value, m.unit)}</span>
          </div>
      ...
      {/* Sparkline chart (if multiple readings) */}
      {sorted.length >= 2 ? (
        <MiniSparkline history={sorted} color={catColor} refMin={m.ref_min} refMax={m.ref_max} unit={m.unit} />
      ) : (
...
export function Biomarkers({ embedded }: { embedded?: boolean } = {}) {
  ...
  // Build history lookup: marker_canonical -> sorted readings
  const historyByMarker = allData.reduce<Record<string, BiomarkerReading[]>>((acc, r) => {
    (acc[r.marker_canonical] ??= []).push(r); return acc;
  }, {});

  // Adviser panel data
  const adviserStart = uploads.length > 0 ? uploads[uploads.length - 1].test_date || "2020-01-01" : "2020-01-01";
  const adviserEnd = uploads[0]?.test_date || new Date().toISOString().slice(0, 10);
  const gatherAdviserData = useCallback(() => {
    const summary: Record<string, unknown> = {
      screening_dates: [...new Set(allData.map(r => r.test_date))].sort(),
      total_markers: latestList.length,
      out_of_range: latestList.filter(m => m.status !== "normal").map(m => ({
        marker: m.marker_canonical,
        value: m.value,
        unit: m.unit,
        status: m.status,
        ref_range: m.ref_min != null && m.ref_max != null ? `${m.ref_min}-${m.ref_max}` : null,
      })),
    };
    const byCat: Record<string, { marker: string; value: number; unit: string; status: string; trend?: string }[]> = {};
    for (const m of latestList) {
      const cat = m.category;
      if (!byCat[cat]) byCat[cat] = [];
      const hist = allData.filter(r => r.marker_canonical === m.marker_canonical).sort((a, b) => a.test_date.localeCompare(b.test_date));
      const trend = hist.length >= 2
        ? hist.map(h => `${h.test_date}: ${h.value}`).join(" → ")
        : undefined;
      byCat[cat].push({ marker: m.marker_canonical, value: m.value, unit: m.unit, status: m.status, trend });
    }
    summary.markers_by_category = byCat;
    return summary;
  }, [allData, latestList]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      {latestList.length > 0 && (
        <AdviserPanel
          tab="blood tests / biomarkers"
          start={adviserStart}
          end={adviserEnd}
          gatherData={gatherAdviserData}
        />
      )}
      ...
      {latestList.length > 0 && (
        <div style={{ display: "grid", gridTemplateColumns: "180px 1fr", gap: "1rem" }}>
          ...
          {/* ── Marker cards grid ── */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: "0.6rem", alignContent: "start" }}>
            {displayMarkers.map(m => (
              <MarkerCard
                key={m.marker_canonical}
                m={m}
                history={historyByMarker[m.marker_canonical] ?? []}
